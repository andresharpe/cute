...
{
    public static List<ReadOnlyMemory<char>> GetFixedSpanLines(this ReadOnlySpan<char> input, int maxLength = 80, int? maxFirstLineLength = null)
    {
        ...
        if (maxFirstLineLength is not null && input.Length > maxFirstLineLength)
        {
            ...
            if (lastBreakIndex == -1)
            {
                lines.Add(ReadOnlyMemory<char>.Empty);
        ...
        while (!input.IsEmpty)
        {
            ...
            {
                lines.Add(slice[..firstNewlineIndex].ToArray());
